...
export class EmulationOrganizer implements IEmulationOrganizer {
  ...
  init(): Result<void, EmulationOrganizerError> {
    ...
    for (let i = 0; i < iterationLimit; i++) {
      ...
      if (!res.ok) {
        ...
        return {
          ok: false,
          error: new EmulationOrganizerError("Failed to initialize emulation client.", { cause: res.error }),
        };
      }
  ...
  eval(
    ...
    for (let i = 0; i < duration; i++) {
      ...
      const res = this.circuitEmulatorService.eval(inputRecord);
      if (!res.ok) {
        const err = new EmulationOrganizerError("Failed to evaluate circuit.", { cause: res.error });
        console.error(err);
    ...
    if (!finalOutputRecord) {
      const err = new EmulationOrganizerError("No final output record found.");
      console.error(err);
      return { ok: false, error: err };
    }
