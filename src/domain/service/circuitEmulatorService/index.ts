...
export class CircuitEmulatorService implements ICircuitEmulatorService {
  ...
  static from(circuitGraphData: CircuitGraphData): Result<CircuitEmulatorService> {
    ...
    removedNodeIds.forEach((nodeID, idx) => {
      ...
      if (!target) return { ok: false, error: new CircuitEmulatorServiceFromError(`Node not found: ${nodeID}`) };
      ...
      for (const node of this.nodes) {
        ...
        switch (true) {
          case type === "ENTRY": {
            ...
            if (!currentOutput.ok)
              return { ok: false, error: new CircuitEmulatorServiceSetupError(`Failed to setup ENTRY node: ${id}`) };
          ...
          default: {
            ...
            if (!incomingInputRecord.ok) return { ok: false, error: incomingInputRecord.error };
      ...
      for (const node of this.nodes) {
        ...
        switch (type) {
          case "ENTRY": {
            ...
            if (!currentOutput.ok)
              return { ok: false, error: new CircuitEmulatorServiceEvalError(`Failed to setup ENTRY node: ${id}`) };
  ...
  getInfomationById(id: CircuitNodeId): Result<NodeInformation> {
    ...
    return node
      ? { ok: true, value: node }
      : { ok: false, error: new CircuitEmulatorServiceGetInfomationByIdError(`Node not found: ${id}`) };
  }
  ...
  private referIncomingInputs(nodeID: CircuitNodeId, currentPhase: Phase, currentTick: Tick): Result<InputRecord> {
    ...
    if (!node) return { ok: false, error: new CircuitEmulatorServiceReferIncomingInputs(`Node not found: ${nodeID}`) };

    const { type, inputs } = node.getInformation();
    if (type === "ENTRY")
      return {
        ok: false,
        error: new CircuitEmulatorServiceReferIncomingInputs(
    ...
    for (const input of inputs) {
      ...
      if (!targetInfo)
        return { ok: false, error: new CircuitEmulatorServiceReferIncomingInputs("Target node not found") };
        ...
        case targetInfo.history.get(currentPhase)?.get(currentTick) === undefined && isFirstTick === false: {
          ...
          if (output === undefined)
            return {
              ok: false,
              error: new CircuitEmulatorServiceReferIncomingInputs(
                `No output found for node: ${nodeID}, phase: ${currentPhase}, tick: ${currentTick - 1}`,
              ),
            };
        ...
        default: {
          ...
          if (output === undefined)
            return {
              ok: false,
              error: new CircuitEmulatorServiceReferIncomingInputs(
                `No output found for node: ${nodeID}, phase: ${currentPhase}, tick: ${currentTick}`,
              ),
            };
          inputRecord[CircuitNodeInputId.from(targetInfo.id)] = output;
          break;
