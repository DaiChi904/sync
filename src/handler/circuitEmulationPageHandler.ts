...
export const useCircuitEmulationPageHandler = ({
  ...
  const fetch = useCallback(async (): Promise<void> => {
    await Attempt.proceed(
      async () => {
        ...
        if (!circuitDetail.ok) {
          throw new Attempt.Abort("Failed to get circuit detail.", {
        ...
        if (!circuitGuiData.ok) {
          throw new Attempt.Abort("Failed to parse circuit data to gui data.", {
        ...
        if (!circuitGraphData.ok) {
          throw new Attempt.Abort("Failed to parse circuit data to graph data.", {
        ...
        if (!circuitEmulatorService.ok) {
          throw new Attempt.Abort("Failed to generate circuit emulator service.", {
  ...
  const setupCircuitEmulatorServiceClient = useCallback((): void => {
    Attempt.proceed(
      () => {
        if (!client) throw new Attempt.Abort("EmulatorService is not defined.", { tag: "noClient" });

        const res = client.setup();
        if (!res.ok) {
          throw new Attempt.Abort("Failed to setup emulator service.", {
            cause: res.error,
            tag: "setupClient",
          });
        }
  ...
  const registInputNodes = useCallback((): void => {
    ...
    Attempt.proceed(
      () => {
        if (!guiData) throw new Attempt.Abort("GuiData is not defined.", { tag: "noGuiData" });
  ...
  const registOutputs = useCallback((): void => {
    Attempt.proceed(
      () => {
        if (!client) throw new Attempt.Abort("EmulatorService is not defined.", { tag: "noClient" });
        if (!availableNodeIds)
          throw new Attempt.Abort("AvailableNodeIds is not defined.", { tag: "noAvailableNodeIds" });
        ...
        availableNodeIds.forEach((nodeId) => {
          ...
          if (!res.ok) {
            throw new Attempt.Abort("Failed to get information by id.", {
              cause: res.error,
          ...
          if (!source) {
            throw new Attempt.Abort("No history found for the current phase", {
              tag: "generatingOutputRecord",
            });
          }
          const output = Array.from(source).at(-1);
          if (!output) {
            throw new Attempt.Abort("No output found for the current phase", {
              tag: "generatingOutputRecord",
            });
          }
  ...
  const evalCircuit = useCallback((): void => {
    Attempt.proceed(
      () => {
        ...
        if (!res.ok) {
          throw new Attempt.Abort("Failed to eval circuit.", {
            cause: res.error,
            tag: "evalCircuit",
          });
        }
